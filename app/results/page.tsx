...
export default function Results() {
  ...
  const apiEndpoint = 'https://lm6vr3st47.execute-api.us-east-1.amazonaws.com/default/test'
  ...
  return (
    <motion.div initial='hidden' animate='visible' variants={fadeInUp}>
      <>
        <main>
          <div className='bg-gradient-to-b from-gray-500 to-gray-700 shadow  min-h-screen '>
            <div className='relative isolate overflow-hidden pt-6'>
              {/* Secondary navigation */}
              <header className='pb-4 pt-6 sm:pb-6'>
                <div className='mx-auto flex max-w-7xl flex-wrap items-center gap-6 px-4 sm:flex-nowrap sm:px-6 lg:px-8'>
                  <h1 className='text-2xl font-semibold leading-7 text-gray-300'>Results</h1>
                  <div className='order-last flex w-full gap-x-8 text-sm font-semibold leading-6 sm:order-none sm:w-auto sm:border-l sm:border-gray-200 sm:pl-6 sm:leading-7'>
                    {secondaryNavigation.map(item => (
                      <a
                        key={item.name}
                        href={item.href}
                        className={item.current ? 'text-indigo-600' : 'text-gray-300'}
                      >
                        {item.name}
                      </a>
                    ))}
                ...
                <dl className='mx-auto grid max-w-7xl grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:px-2 xl:px-0'>
                  {stats.map((stat, statIdx) => (
                    ...
                    >
                      ...
                      <dd
                        className={classNames(
                          stat.changeType === 'negative' ? 'text-green-400' : 'text-gray-300',
                          'text-xs font-medium',
                          stat.changeType === 'positive' ? 'text-rose-600' : 'text-gray-300',
                          'text-xs font-medium'
                        )}
                      >
                        {stat.change}
                    ...
                    {results
                      ...
                      .map((item, index) => (
                        ...
                        >
                          ...
                          <h2 style={{ fontSize: '18px', marginBottom: '10px' }}>User ID: {item.userId}</h2>
